# ClusteredLightManager3D — Implementation Plan (Refined Production Blueprint)

This document outlines the refined technical architecture, mathematical formulations, WebGL2 texture packing, global ShaderChunk injection mechanics, and implementation phases for **ClusteredLightManager3D**.

---

## 1. System Architecture & Lifecycle

`ClusteredLightManager3D` operates as a dual-component subsystem optimized for maximum CPU efficiency ($< 0.03\text{ ms}$), zero shader recompilations, and complete cross-platform WebGL2 reliability:
...
sequenceDiagram
    ...
    participant CLM as ClusteredLightManager3D (CPU)
    participant GPU as WebGL2 Texture Buffers
    participant Mat as Global PBR Shaders
    
    Note over CLM: Startup: Precompute 3,456 View-Space Cluster AABBs
    GD->>CLM: doStepPostEvents (Camera & Lights positioned)
    CLM->>CLM: Transform active lights to Camera View Space (V * P)
    CLM->>CLM: Broadphase Slice Bounds [Zmin..Zmax, Xmin..Xmax, Ymin..Ymax]
    CLM->>CLM: Fast Arvo Sphere-to-AABB Culling (0.03ms)
    CLM->>GPU: Upload texSubImage2D / texSubImage3D (< 50 KB)
...
2. uClusterGrid (Dual Mode):
   Mode A (Desktop/Modern Mobile): Data3DTexture RG16UI (16 x 9 x 24 texels)
   Mode B (Universal 2D Fallback): DataTexture RG16UI (144 x 24 texels, where index = cX + cY * 16)
   Channels:
...
```mermaid
gantt
    title ClusteredLightManager3D Implementation Roadmap
