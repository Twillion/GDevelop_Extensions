...
console.log('--- Test 42: persistent foam is opt-in, decays in real seconds, and falls back ---');
{
  ...
  // Default OFF, and the shader term is inert.
  {
    ...
    assert.strictEqual(o.persistentFoam, false, 'persistent foam must be off unless asked for');
    assert.strictEqual(o.foamRT, null, 'and no render target is allocated');
    assert.strictEqual(o.material.uniforms.u_FoamBufferOn.value, 0.0,
      'the shader term is switched off, so the crest mask is purely of this frame');
    FW.updateOceanField(o, 1.0);
    assert.strictEqual(o.material.uniforms.u_FoamBufferOn.value, 0.0, 'and stays off after a step');
  }
...
console.log('--- Test 43: foam coverage tracks the Beaufort rung instead of whiting out the sea ---');
{
  ...
  const [tHi, tSlope] = readPair(
    /float crestThreshold = max\(([\d.]+) - u_FoamCoverage \* ([\d.]+)/, 'the crest threshold');
  ...
  const measure = (scaleName) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // vPeak is elevation above the still plane, matching the vertex shader.
      const z = o.field.height[i] + (o.field1 ? o.field1.height[i] * cascade : 0);
      if (z / Math.max(peakRef, 1) >= 0.999) peakSat++;
...
console.log('--- Test 44: foam breaks ON the crests, not in the troughs ---');
{
  ...
  for (const scaleName of ['Beaufort 4 - Moderate Breeze', 'Beaufort 6 - Strong Breeze',
    ...
    const r = correlate(fold, elevation);
    assert.ok(r > 0.35, rung + ': corr(fold, elevation) = ' + r.toFixed(3) +
      ' - foam must break on the crests. Negative means the displacement spectrum sign is flipped ' +
      '(see OceanField.evolve); near zero means choppiness is not reaching the fold.');
  ...
  // The crest mask that drives subsurface and glitter must read elevation, not the horizontal
  // offset - |displacement| measured a correlation of 0.086 with elevation, i.e. nearly flat.
  const wv = runtimeCode.slice(runtimeCode.indexOf('WAVEWORKS_VERTEX_SHADER'),
    runtimeCode.indexOf('WAVEWORKS_FRAGMENT_SHADER'));
  assert.ok(/vPeak = clamp\(totalDisp\.z/.test(wv),
    'vPeak must be taken from elevation; length(totalDisp.xy) is almost uniform over the field');

  // And the fold has to describe the surface actually drawn, which is displaced by u_Choppiness.
  assert.ok(/computeFoam = function \(out, choppiness\)/.test(runtimeCode),
    'computeFoam must take the choppiness the shader applies, or it measures a different surface');
