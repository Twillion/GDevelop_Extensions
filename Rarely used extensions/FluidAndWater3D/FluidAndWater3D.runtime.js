...
/**
  ...
  var WAVEWORKS_FRAGMENT_SHADER = [
    ...
    'uniform float u_FresnelMax;',  // ceiling on the sky mirror, so water colour always shows
    ...
    '  // How much water one pixel covers, against the finest detail the field actually carries.',
    '  // Past the distance where that detail stops resolving, we are shading a whole patch of sea',
    '  // with ONE smooth normal - so a big wave face behaves like a mirror and the reflection grows',
    '  // with the wave instead of breaking up. Real water never gets that clean: the ripples are',
    '  // still there, just smaller than a pixel, and their spread of normals both widens the',
    '  // highlight and lowers the average reflectance. This is that missing variance.',
    '  vec2 pixelSpan = fwidth(vGdXY);',
    '  float pixelWorld = max(length(pixelSpan), 0.0001);',
    '  float texelWorld = max(u_CascadeTileSize * u_CascadeTexel, 0.0001);',
    '  float subPixel = clamp(pixelWorld / (texelWorld * 2.0), 0.0, 1.0);',
    '',
    '  // Schlick, with a ceiling. Physically this reaches 1.0 at grazing and the surface becomes a',
    '  // perfect mirror - which, against a procedural sky with no clouds and no structure, is a',
    '  // large flat wash of one colour. Holding some water colour back is what keeps the sea',
    '  // reading as water from a low camera.',
    '  float fresnel = min(0.02 + 0.98 * pow(1.0 - NdotV, 5.0), u_FresnelMax);',
    '  // Averaged over a spread of normals the reflectance is well below its grazing peak, so a',
    '  // sub-pixel patch must not be given the peak value of a single smooth one.',
    '  fresnel = mix(fresnel, fresnel * 0.42, subPixel);',
    '',
    ...
    '  // Optical path, not volume thickness. `column` is the water body Z size, and a body 3568',
    '  // units deep against a 130-unit extinction depth is 27 extinction lengths - utterly opaque,',
    '  // identical in every pixel. The shallow colour then never appears, the water contributes no',
    '  // variation at all, and the surface becomes a flat mirror of the sky. Clamp how far the',
    '  // colour is allowed to travel so the SHAPE of the volume stops deciding the look.',
    '  float pathDepth = min(column, u_ExtinctionDepth * 1.5);',
    '  // And make it view-dependent: looking down you see through a little water, looking along it',
    '  // you see through a lot. That view term is where an ocean gets its near-dark, far-bright',
    '  // gradient, and with a constant column there was none.',
    '  float viewPath = 1.0 / max(NdotV, 0.20);',
    '  float depthUnits = min((pathDepth / max(u_ExtinctionDepth, 1.0)) * viewPath, 4.0);',
    '  vec3 transmit = exp(-BETA_EXTINCTION * depthUnits * (6.0 * contrast));',
    ...
    '  // Same variance, applied to the highlight: where the ripples are finer than a pixel the',
    '  // lobe has to widen and dim, or one smooth wave face returns a single mirror-bright blob',
    '  // whose SIZE grows with the wave rather than staying the size of a glint.',
    '  specRough = mix(specRough, 6.0, subPixel);',
    '  specInt = specInt * mix(1.0, 0.22, subPixel);',
    ...
    '  // Opacity follows how much water is actually BELOW this point, not the clamped optical path',
    '  // the colour uses. They were the same number until that path was clamped, at which point',
    '  // transmit stopped being near zero and deep ocean quietly became 60% transparent - the',
    '  // background then showed through as a pale wash that survived turning off foam, specular and',
    '  // the sky reflection, because it was never any of them.',
    '  float depthOpacity = clamp(column / max(u_ExtinctionDepth * 0.75, 1.0), 0.0, 1.0);',
    '  float alpha = mix(0.40, 0.98, depthOpacity);',
