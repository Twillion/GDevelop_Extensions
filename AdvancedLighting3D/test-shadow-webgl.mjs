...
window.run=async function(){
 ...
 AL.setShadowMode(scene,'CSM');draw();draw();const csm=pixels();
 ...
 AL.setShadowMode(scene,'Hybrid');draw();draw();const hybrid=pixels();let hybridDifference=0;for(let i=0;i<csm.length;i++)hybridDifference=Math.max(hybridDifference,Math.abs(csm[i]-hybrid[i]));
 ...
 AL.setShadowMode(scene,'SDF');draw();draw();const sdf=AL.isSDFBakeComplete(scene);const sdfPixels=pixels();let sdfChanged=0;for(let i=0;i<off.length;i+=4)if(Math.abs(off[i]-sdfPixels[i])>5)sdfChanged++;
 ...
 return {revision:THREE.REVISION,ready,contain,changed,hybridDifference,count4,count2,sdf,sdfChanged,steps,restoreDifference,noCascades,glErrors,image:saved,glError:renderer.getContext().getError()};
};</script>`;
const server=http.createServer((req,res)=>{res.setHeader('Content-Type',req.url==='/'?'text/html':'text/javascript');if(req.url==='/')res.end(html);else if(req.url==='/three.js')res.end(fs.readFileSync(path.join(root,'Rarely used extensions/gdjs-harness/runtime/pixi-renderers/three.js')));else if(req.url==='/runtime.js')res.end(fs.readFileSync(path.join(root,'AdvancedLighting3D/AdvancedLighting3D.runtime.js')));else{res.statusCode=204;res.end();}});
