...
`, { group: G_MASTER }),
  ...
  fn('SetColorGradingLUT', 'Set colour grading LUT',
    'Set colour grading LUT on _PARAM0_ to _PARAM2_',
    'Apply a colour grading lookup table from a LUT strip image. The strip must be N tiles across, each N by N pixels, so 256x16, 1024x32 or 4096x64. Blue is the tile index, red runs left to right within a tile, green runs top to bottom. The table is applied AFTER tone mapping, in the sRGB domain that grading tools export, which is the domain a .cube-to-strip export expects. Contract: sRGB in, sRGB out, display-referred, Rec.709 primaries. Log and ACES LUTs are NOT supported and will grade incorrectly. Pass an empty name to remove the LUT.',
    'Action',
    [{ name: 'Resource', type: 'imageResource', description: 'LUT strip image' }],
    `const val = eventsFunctionContext.getArgument("Resource");
if (behavior._setColorGradingLUT) behavior._setColorGradingLUT(val);
FX.updateSettings(runtimeScene, behavior, { lutResource: val });
`, { group: G_MASTER }),

  fn('SetColorGradingIntensity', 'Set colour grading intensity',
    'Set colour grading intensity on _PARAM0_ to _PARAM2_',
    'Blend between the ungraded tone-mapped image and the LUT result, from 0 to 1. 0 bypasses the table entirely and costs nothing.', 'Action',
    [num('Intensity', 'Grading intensity (0 to 1)', '1')],
    `const val = eventsFunctionContext.getArgument("Intensity");
if (behavior._setColorGradingIntensity) behavior._setColorGradingIntensity(val);
FX.updateSettings(runtimeScene, behavior, { lutIntensity: val });
`, { group: G_MASTER }),
...
const cinematicPostFXBehavior = {
  ...
  propertyDescriptors: [
    ...
    prop('ColorGradingLUT', 'Resource', 'Colour Grading LUT',
      'LUT strip image for colour grading, applied after tone mapping. The strip must be N tiles across, each N by N pixels: 256x16, 1024x32 or 4096x64. Blue is the tile index, red runs left to right within a tile, green top to bottom. Contract: sRGB in, sRGB out, display-referred, Rec.709. Log and ACES LUTs are not supported and will grade incorrectly. Leave empty for no grading.',
      '',
      { group: G_MASTER, extraInformation: ['image'] }),
    prop('ColorGradingIntensity', 'Number', 'Colour Grading Intensity',
      'Blend between the ungraded tone-mapped image and the LUT result, 0 to 1. 0 is a true bypass.',
      '1.0',
      { group: G_MASTER }),
