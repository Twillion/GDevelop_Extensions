...
const rawSpecularAARuntime = fs.readFileSync(path.join(here, 'SpecularAA3D.runtime.js'), 'utf8');
...
/* ================================================================= Specular AA behavior */
// Tokuyoshi & Kaplanyan 2019 geometric specular anti-aliasing. Shader-level only: it never writes a
// material field, so it does not go through the Core contributor negotiation.
const SPECAA_BEHAVIOR_NAME = 'SpecularAA3D';
const SPECAA_BEHAVIOR_TYPE = `${EXTENSION_NAME}::${SPECAA_BEHAVIOR_NAME}`;
const SPECAA_NS = 'gdjs.__m3dSpecularAA';
const SPECAA_OB = [
  { name: 'Object', type: 'object', description: 'Object' },
  { name: 'Behavior', type: 'behavior', description: 'Behavior', supplementaryInformation: SPECAA_BEHAVIOR_TYPE },
];
const specaaRuntime = rawChainRuntime + '\n' + rawSpecularAARuntime;
const safn = (name, fullName, sentence, description, functionType, parameters, code, opts = {}) => ({
  name, fullName, sentence, description, functionType,
  ...(opts.group ? { group: opts.group } : {}), private: opts.private === true,
  parameters: [...SPECAA_OB, ...parameters],
  events: [{ type: 'BuiltinCommonInstructions::JsCode',
    inlineCode: (opts.withRuntime ? specaaRuntime + '\n' : '') + code, parameterObjects: 'Object' }],
});
// Re-applied every step rather than only on change: materials are swapped and re-isolated by other
// Material 3D behaviors, and a settings object attached to a material that has since been replaced
// silently stops filtering. applyToMaterial only forces a recompile when source-affecting flags
// actually differ, so re-applying a steady state costs nothing.
const SPECAA_SYNC = `if(!${SPECAA_NS})return;var o=eventsFunctionContext.getObjects("Object");` +
  `var n=eventsFunctionContext.getBehaviorName("Behavior");` +
  `for(var i=0;i<o.length;i++){var b=o[i].getBehavior(n);if(!b)continue;` +
  `${SPECAA_NS}.applyToObject(o[i],{enabled:b._getEnabled(),sigma2:b._getFilterWidth(),` +
  `kappa:b._getClampThreshold(),geometricFloor:b._getIncludeGeometricCurvature()});}\n`;
const specaaFunctions = [
  safn('doStepPreEvents', 'doStepPreEvents', '', '', 'Action', [], SPECAA_SYNC,
    { withRuntime: true, private: true }),
  safn('onDestroy', 'onDestroy', '', '', 'Action', [],
    `if(!${SPECAA_NS})return;var o=eventsFunctionContext.getObjects("Object");` +
    `for(var i=0;i<o.length;i++)${SPECAA_NS}.removeFromObject(o[i]);\n`, { private: true }),
  safn('SetEnabled', 'Enable specular anti-aliasing', 'Enable specular anti-aliasing on _PARAM0_: _PARAM2_',
    'Toggle geometric specular anti-aliasing for this object.', 'Action',
    [{ name: 'Value', type: 'yesorno', description: 'Enable' }],
    `if(!${SPECAA_NS})return;var o=eventsFunctionContext.getObjects("Object");var n=eventsFunctionContext.getBehaviorName("Behavior");` +
    `for(var i=0;i<o.length;i++){var b=o[i].getBehavior(n);if(b)b._setEnabled(eventsFunctionContext.getArgument("Value"));}\n`,
    { group: 'Specular AA' }),
  safn('SetFilterWidth', 'Set filter width', 'Set specular AA filter width on _PARAM0_ to _PARAM2_',
    'Pixel filter kernel variance. Higher removes more sparkle, but blurs highlights and drifts further from a correctly supersampled image. The 0.02 default was measured against a 16x supersampled reference, not guessed. Halving this gives the paper\'s less conservative Equation 5.',
    'Action', [num('Value', 'Filter width (0 to 4)', '0.02')],
    `if(!${SPECAA_NS})return;var o=eventsFunctionContext.getObjects("Object");var n=eventsFunctionContext.getBehaviorName("Behavior");` +
    `for(var i=0;i<o.length;i++){var b=o[i].getBehavior(n);if(b)b._setFilterWidth(eventsFunctionContext.getArgument("Value"));}\n`,
    { group: 'Specular AA' }),
  safn('SetClampThreshold', 'Set clamp threshold', 'Set specular AA clamp threshold on _PARAM0_ to _PARAM2_',
    'Upper bound on how much roughness the filter may add, in the alpha-squared domain. 0.18 is the value stated in the paper.',
    'Action', [num('Value', 'Clamp threshold (0 to 1)', '0.18')],
    `if(!${SPECAA_NS})return;var o=eventsFunctionContext.getObjects("Object");var n=eventsFunctionContext.getBehaviorName("Behavior");` +
    `for(var i=0;i<o.length;i++){var b=o[i].getBehavior(n);if(b)b._setClampThreshold(eventsFunctionContext.getArgument("Value"));}\n`,
    { group: 'Specular AA' }),
  safn('IsFiltering', 'Specular AA is active', '_PARAM0_ specular anti-aliasing is active',
    'True when the shader filter is installed on this object materials. False for unlit materials, which have no lighting stage to filter.',
    'Condition', [],
    `var o=eventsFunctionContext.getObjects("Object"),r=false;` +
    `if(o.length&&${SPECAA_NS}){var root=o[0].get3DRendererObject?o[0].get3DRendererObject():null;` +
    `if(root&&root.traverse)root.traverse(function(nd){if(r||!nd.isMesh||!nd.material)return;` +
    `var ms=Array.isArray(nd.material)?nd.material:[nd.material];` +
    `for(var i=0;i<ms.length;i++){var s=${SPECAA_NS}.settingsOf(ms[i]);if(s&&s.enabled)r=true;}});}` +
    `eventsFunctionContext.returnValue=r;\n`, { group: 'Diagnostics' }),
];
const specaaBehavior = {
  name: SPECAA_BEHAVIOR_NAME, fullName: 'Specular Anti-Aliasing 3D',
  description: 'Removes specular sparkle and shimmer from rough and normal-mapped surfaces using Tokuyoshi and Kaplanyan 2019 filtering. Replaces the cruder estimate Three.js applies by default, and filters clearcoat as well. Does not fix aliasing from geometric edges, and cannot recover normal-map detail already lost to mipmapping.',
  objectType: '',
  propertyDescriptors: [
    brdfProp('Enabled', 'Boolean', 'Enabled', 'Filter specular highlights on this object.', 'true', { group: 'Specular AA' }),
    brdfProp('FilterWidth', 'Number', 'Filter width',
      'Pixel filter kernel variance. Higher removes more sparkle, but blurs highlights and drifts further from a correctly supersampled image. The default of 0.02 was measured, not guessed: on a metal sphere array with a mipmapped normal map, checked against a 16x supersampled reference, every value above roughly 0.02 made both shimmer AND accuracy worse, and Filament\'s 0.15 failed badly. Halving this gives the paper\'s less conservative Equation 5, which is why there is no separate mode switch. Re-measure on your own content with demos/bench/run-sigma-sweep.mjs.',
      '0.02', { group: 'Specular AA' }),
    brdfProp('ClampThreshold', 'Number', 'Clamp threshold',
      'Upper bound on how much roughness the filter may add, in the alpha-squared domain. 0.18 is the value stated in the paper. Lower keeps highlights sharper at the cost of more sparkle.',
      '0.18', { group: 'Specular AA' }),
    brdfProp('IncludeGeometricCurvature', 'Boolean', 'Also filter geometric curvature',
      'Additionally run the filter on the geometric normal and take the stronger of the two. Restores a conservative floor against curvature that the shading normal alone does not see. Costs two extra derivative pairs per pixel.',
      'false', { group: 'Specular AA' }),
  ],
  eventsFunctions: specaaFunctions,
};
...
**Alpha Mode** and **Rendered Side** now default to **Preserve**. Previously attaching the behavior silently flattened transparency on models that shipped with it.`,
  ...
  eventsBasedBehaviors: [behavior, physicalBehavior, animatedBehavior, patternBehavior, wetBehavior, brdfBehavior, displacedBehavior, specaaBehavior],
  eventsBasedObjects: [],
};
...
checkControlChars(rawSpecularAARuntime, 'SpecularAA3D.runtime.js');
...
for (const [src, name] of [[rawRuntime, 'MaterialMaster.runtime.js'], [rawPhysicalRuntime, 'PhysicalMaterial3D.runtime.js'], [rawAnimatedRuntime, 'AnimatedMaterial3D.runtime.js'], [rawPatternRuntime, 'PatternMaterial3D.runtime.js'], [rawWetRuntime, 'WetMaterial3D.runtime.js'], [rawBrdfRuntime, 'BRDFMaterial.runtime.js'], [rawSpecularAARuntime, 'SpecularAA3D.runtime.js']]) {
