...
### Shared with AdvancedLighting3D

`ShaderChain.runtime.js` is embedded in **both** this extension and AdvancedLighting3D, which
registers its clustered-lighting, probe and SDF-shadow injection in band 100. The copies negotiate by
`CHAIN_VERSION` at load: a newer copy replaces an older one, adopts its injector registry and
re-installs on materials the old copy had patched. Either extension works installed alone, and either
load order gives the same result.

`ensureAll()` runs once per frame from AdvancedLighting3D's post-events tick. Several runtimes —
including Material 3D itself — swap in a fresh material rather than mutating the existing one, which
discards the hook and the chain state with the old object and reports nothing. `ensureAll()` re-patches
anything that lost the hook since the previous frame.

> **Known incompatibility:** `Portal3D` assigns `onBeforeCompile` directly and is not part of the
> chain. On any material it shares with this extension or AdvancedLighting3D, whichever patches last
> wins and the other's edits silently vanish. Do not combine Portal3D with Material 3D or
> AdvancedLighting3D on the same object.
