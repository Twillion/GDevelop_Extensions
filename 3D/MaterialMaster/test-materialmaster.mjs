...
console.log('\n27. Inspector panel preset & primary selector positioning and preset resolution');
{
  ...
  // A file covered by its own dedicated suite counts as loaded: SpecularAA3D needs real GL (and a
  // headless browser) to be tested honestly, which does not belong in this Node suite.
  const coveredElsewhere = { 'SpecularAA3D.runtime.js': 'test-specular-aa.mjs' };
  for (const [file, suite] of Object.entries(coveredElsewhere)) {
    check(`${file} has its dedicated suite on disk`, fs.existsSync(path.join(here, suite)), suite);
  }
  const notLoaded = onDisk.filter((f) => !suiteSrc.includes(`'${f}'`) && !coveredElsewhere[f]);
...
console.log('\n34. Shader chain v2 — tracking, replacement recovery, upgrade');
{
  const chain = globalThis.gdjs.__m3dShaderChain;

  check('chain reports its version', chain.VERSION === 2, String(chain.VERSION));
  check('registry and tracking live on shared state, not in the closure',
    !!globalThis.gdjs.__m3dShaderChainState &&
    Array.isArray(globalThis.gdjs.__m3dShaderChainState.injectors) &&
    Array.isArray(globalThis.gdjs.__m3dShaderChainState.tracked));

  chain.register({
    id: 'v2-probe', chunk: 'map_fragment', order: 450,
    isActive: (m) => m.__v2Probe === true,
    key: () => 'v1',
    inject: (shader) => { shader.fragmentShader += '\n// v2-probe'; },
  });

  // --- install tracks
  const before = chain.trackedCount();
  const tracked = new MeshStandardMaterial();
  tracked.__v2Probe = true;
  chain.install(tracked);
  check('install() adds the material to the tracked list',
    chain.trackedCount() === before + 1, `${before} -> ${chain.trackedCount()}`);
  check('installing the same material twice does not double-track',
    (chain.install(tracked), chain.trackedCount()) === before + 1);

  // --- the material-replacement case: a runtime swaps in a fresh material and the hook is gone.
  // Simulated here by stripping the hook the way `new THREE.MeshStandardMaterial()` would.
  tracked.onBeforeCompile = () => {};
  tracked.__m3dChainInstalled = false;
  check('a material that lost its hook reads as not installed',
    chain.isInstalled(tracked) === false);
  check('ensureAll() re-patches it', chain.ensureAll() >= 1);
  check('  and it dispatches injectors again', (() => {
    const shader = { uniforms: {}, fragmentShader: 'void main(){}', vertexShader: '' };
    tracked.onBeforeCompile(shader);
    return chain.injectedIds(tracked).includes('v2-probe');
  })());

  // --- ensureAll must not patch materials no injector claims
  const bystander = new MeshStandardMaterial();
  chain.install(bystander);
  bystander.onBeforeCompile = () => {};
  bystander.__m3dChainInstalled = false;
  chain.ensureAll();
  check('ensureAll() leaves a material with no active injectors alone',
    chain.isInstalled(bystander) === false);

  // --- uninstall releases and untracks
  const disposable = new MeshStandardMaterial();
  disposable.__v2Probe = true;
  chain.install(disposable);
  const trackedWith = chain.trackedCount();
  chain.uninstall(disposable);
  check('uninstall() clears the installed flag', chain.isInstalled(disposable) === false);
  check('uninstall() stops tracking the material',
    chain.trackedCount() === trackedWith - 1, `${trackedWith} -> ${chain.trackedCount()}`);
  check('  and ensureAll() does not resurrect it',
    (chain.ensureAll(), chain.isInstalled(disposable)) === false);

  // --- clone safety, restated against v2
  const cloned = tracked.clone();
  check('a clone drops the chain hook', chain.isInstalled(cloned) === false);
  cloned.__v2Probe = true;
  check('ensure() re-installs on the clone', chain.ensure(cloned) === true);

  // --- unregister
  chain.register({ id: 'v2-temp', chunk: 'map_fragment', order: 460,
    isActive: () => false, key: () => 'v1', inject: () => {} });
  check('unregister() removes an injector',
    chain.unregister('v2-temp') === true && chain.registeredIds().indexOf('v2-temp') < 0);

  // --- UPGRADE: a newer copy replaces this one and must re-bind already-installed materials.
  // Their hooks close over the OLD module's dispatch, so a version bump has to re-install them
  // rather than trust the __m3dChainInstalled flag they still carry.
  const survivor = new MeshStandardMaterial();
  survivor.__v2Probe = true;
  chain.install(survivor);
  const hookBefore = survivor.onBeforeCompile;

  globalThis.gdjs.__m3dShaderChain.VERSION = 1;          // pretend the resident copy is older
  new Function('runtimeScene', 'eventsFunctionContext', chainSrc)(null, null);
  const upgraded = globalThis.gdjs.__m3dShaderChain;

  check('a newer copy replaces an older one', upgraded.VERSION === 2);
  check('  and inherits the shared injector registry',
    upgraded.registeredIds().includes('v2-probe'));
  check('  and re-installs materials the old copy had patched',
    survivor.onBeforeCompile !== hookBefore);
  check('  so those materials still dispatch correctly', (() => {
    const shader = { uniforms: {}, fragmentShader: 'void main(){}', vertexShader: '' };
    survivor.onBeforeCompile(shader);
    return upgraded.injectedIds(survivor).includes('v2-probe');
  })());

  // An equal version must NOT replace — otherwise every extension embedding this file would
  // reinstall over the others on every load.
  const sameRef = globalThis.gdjs.__m3dShaderChain;
  new Function('runtimeScene', 'eventsFunctionContext', chainSrc)(null, null);
  check('an equal version leaves the resident copy in place',
    globalThis.gdjs.__m3dShaderChain === sameRef);
}
