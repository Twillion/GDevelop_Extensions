...
Add this behavior to any 3D object (typically a **Cube3D** or anchor object) to define an authored decal volume in the scene editor. The cube's scale defines the decal's width ($X$), height ($Y$), and projection depth ($Z$). The anchor object can be hidden using a standard visibility action; its world transform will still drive the decal volume.
...
| `LayerMask` | Number | `1` | 16-bit receiver/decal matching mask (bits 0–15). |
| `Roughness` | Number | `-1.0` | PBR roughness override ($0.0$ to $1.0$); `-1.0` leaves underlying material roughness unchanged. |
| `Metalness` | Number | `-1.0` | PBR metalness override ($0.0$ to $1.0$); `-1.0` leaves underlying material metalness unchanged. |
...
| `LayerMask` | Number | `1` | Bitmask for layer filtering (e.g., allow blood decals on props but not on water). Decals project only onto surfaces whose `LayerMask` shares at least one active bit with the decal's mask (`(decalMask & receiverMask) != 0`). |
...
* **Parameters:**
  ...
  * `Lifetime` (Number): Seconds before deletion (e.g. $30.0$; negative numbers pin the decal).
  * `FadeDuration` (Number): Seconds of linear fading prior to deletion (e.g. $2.0$).
  * `Layer` (String, optional): 3D layer containing the decals and receiving objects (default: `""`).
...
* **Parameters:**
  ...
  * `Layer` (String, optional): 3D layer name (default: `""`).
...
Assigns the global color/albedo texture atlas resource used for decal projection.
* **Parameters:**
  * `AtlasResource` (Image Resource): Name of image resource in GDevelop project.
  * `Layer` (String, optional): Target 3D layer.

#### `SetDecalNormalAtlasTexture`
Assigns the tangent-space normal map atlas resource used for relief and perturbation details (such as cracks or bullet dents).
* **Parameters:**
  * `AtlasResource` (Image Resource): Normal map atlas image resource.
  * `Layer` (String, optional): Target 3D layer.

#### `SetDecalSurfaceAtlasTexture`
Assigns the PBR surface map atlas (Red channel: roughness, Green channel: metalness).
* **Parameters:**
  * `AtlasResource` (Image Resource): Surface PBR atlas image resource.
  * `Layer` (String, optional): Target 3D layer.
...
#### `SetDecalAppearance`
Updates visual and filtering parameters of a specific decal instance by ID.
* **Parameters:**
  * `DecalId` (Number): ID returned by `SpawnDecal...` or `LastDecalId()`.
  * `Opacity` (Number): Opacity from 0 to 1.
  * `NormalCutoffAngle` (Number): Reject surfaces steeper than this angle in degrees.
  * `EmissiveIntensity` (Number): Emissive radiance multiplier.
  * `Roughness` (Number): Roughness override (-1 to leave unchanged).
  * `Metalness` (Number): Metalness override (-1 to leave unchanged).
  * `LayerMask` (Number): 16-bit receiver/decal matching mask.
  * `Layer` (String, optional): Target 3D layer.

#### `RemoveDecal`
Immediately removes a specific decal by its integer ID.
* **Parameters:**
  * `DecalId` (Number): ID of the decal to remove.
  * `Layer` (String, optional): Target 3D layer.

#### `ClearAllDynamicDecals`
Immediately removes all dynamic (non-pinned) decals from the scene. Pinned decals (such as authored scene decals with `Lifetime = -1`) remain intact.
* **Parameter:** `Layer` (String, optional): Target 3D layer.

#### `SetMaxDecals`
Reallocates the decal buffer capacity.
* **Parameters:**
  * `Capacity` (Number): `64`, `128`, `256`, or `512` (Default: `128`).
  * `Layer` (String, optional): Target 3D layer.
...
| :--- | :--- | :--- |
| `ClusteredDetail::ActiveDecalCount(Layer)` | Number | Current number of active decals rendered this frame. |
| `ClusteredDetail::MaxDecals(Layer)` | Number | Maximum capacity of the decal ring buffer (e.g. 128). |
| `ClusteredDetail::DecalCPUTimeMs(Layer)` | Number | CPU broadphase execution time in milliseconds. |
| `ClusteredDetail::LastDecalId(Layer)` | Number | ID of the most recently spawned decal; returns `-1` if the buffer was full of pinned decals and the spawn was rejected. |
| `ClusteredDetail::ClusterOverflowCount(Layer)` | Number | Count of clusters where decals exceeded the 32-decal cluster limit and oldest overlaps were evicted. |
